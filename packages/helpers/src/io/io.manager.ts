    ...
    public async cpFileStream(
        {
            ...
            from: string
            to: string
            readOptions?: ReadStreamOption
            writeOptions?: WriteStreamOption
        },
        handler?: PromiseCallbacks<ReadStream, Error>
    ): Promisify<ReadStream> {
        const mediaStream = this.reader.readMediaStreamPure({
            filePath: from,
            options: { ...readOptions },
        })

        const writeStream = await this.writer.writeStream({
            filePath: to,
            stream: mediaStream,
            options: { ...writeOptions },
        })

        if (!writeStream.success) {
            if (writeStream.error instanceof Error)
                handler?.onError?.(writeStream.error)

            return {
                success: false,
                error: writeStream.error,
            }
        }
